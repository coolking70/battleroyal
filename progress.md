...
## Phase 4Q — Zone Landmarks, Facilities & Exploration Depth (2026-08-14)

- Added a data-driven 24-landmark registry covering all 12 zones (two per zone), with 9 finite facility interactions, risk-bearing search profiles, deterministic pre-generated ItemStacks, and persisted runtime state for discovery, searches, charges, locks, repair, activation, exhaustion, and last use.
- Added formal `SEARCH_LANDMARK` and `INTERACT_LANDMARK` commands through the shared action-cost, legal-action, actor-action, command-handler, time-advance, save-validation, and item-conservation paths. Landmark loot is finite and hidden until a real search; terminal states reject further use without advancing time.
- Integrated landmark provenance into static/current Craft Guide sources, with current depletion filtering. Added public Codex landmark source names, a current-zone LandmarkPanel, coarse status/charge/search display, targeted command dispatch, and no exact hidden-loot preview.
- Added NPC landmark recommendation and fallback planning, plus a formal AutoPlayer representative route. Focused Phase 4Q tests cover registry thresholds, deterministic targeted search, facilities, source depletion, NPC route/replan, save corruption cases, AutoPlayer commands, and UI information boundaries.
- Verification so far: full Vitest 112 files / 1,643 tests PASS; typecheck/build PASS; dependency audit PASS with R1–R4 zero and maximum audited core/data file at 500 lines; save audit PASS at 109/109 malformed cases rejected and 0 construction failures; art/security gates PASS; networked `npm audit --omit=dev` found 0 vulnerabilities.
- Required 500-game `PHASE4Q` regression generated `reports/phase4q-regression.json`: requested=actual=500, trustworthy=500/500, regressionGate=true, engine health counters zero, timeout=0. Overall balance observation remains false under the existing ratio gate (highest/lowest-nonzero character win-rate ratio 6.5 vs threshold 2.5); this is reported as observation-only and is not used to claim acceptance.
- Browser smoke reached a live new-game screen and the screenshot was visually inspected; no new console error was recorded. Draft PR #24 is open, Draft, and unmerged at implementation head `6e5fdba03fdc8564b4dfaa43971e37533a6de58f`; exact-head CI run `31743004163` completed successfully. Human status remains exactly `NEEDS-HUMAN-PLAYTEST`; remaining work is independent human playtest and review.

## Phase 4Q-AF — Facility Unlock, Landmark Risk & Information-Boundary Closure (2026-08-14)

- Closed secure-storage self-unlock reachability: the formal `requiresUnlock` interaction can execute while locked when its tool, charge, stamina, location, and game-state requirements are satisfied; the field kit is consumed exactly once and save/load preserves the unlocked runtime.
- Closed remote landmark information leakage: static provenance, public remote potential, and actor-local current runtime are now distinct. NPC planning no longer reads remote exhaustion, loot length, lock, disabled state, or last-use state; arrival triggers local stale-plan refresh and fallback.
- Closed lethal landmark reward-after-death: fatal risk resolves before candidate removal/reward dispatch, keeps the hidden UID in landmark loot, leaves pending pickup unchanged, and preserves item integrity. Exhaustion events are actor-scoped rather than default global broadcasts.
- Added 13 AF tests, autonomous cross-zone and hidden-exhaustion fallback route evidence, and the independent `reports/phase4q-af-regression.json` / `.md` 500-game artifact. Full suite is 113 files / 1,656 tests; all engine-health counters are zero.
- Typecheck/build, save/dependency/Art/security gates, CI dry-run art generation, and production `npm audit --omit=dev` passed. Balance remains `BALANCE OBSERVATION ONLY — BALANCE DEFERRED`; old-save migration remains `DEFERRED UNTIL PRE-RELEASE SAVE FORMAT STABILIZATION`; no PNGs changed.
- Status: ready for independent acceptance review. Human status remains exactly `NEEDS-HUMAN-PLAYTEST`; PR #24 remains open, Draft, and unmerged.

## Phase 4Q-AF2 — NPC Landmark Plan Integration & Autonomous Route Closure (2026-08-14)

- Fixed the remaining production gap: fresh and TTL-replanned ordinary NPC recipe plans now commit a deterministic Landmark recommendation together with the normal zone recommendation after recipe selection. Ordinary turns do not rebuild a null recommendation.
- Preserved legacy NPC cadence and route behavior by promoting remote Landmark routing only when the actor's selected current zone is exhausted and the craft plan has no Wild-drop raw gap; objective, Wild, and Phase 4P Apex routes retain their existing planners and null Landmark semantics.
...
## Phase 4S — Actor-Scoped Knowledge, Memory & Strategic Intent (2026-08-15)

- Original Phase 4S prompt is the attached actor-scoped observation, bounded last-known memory, provenance, strategic-intent, save-validation, S-1–S-15, full-gate and 500-game handoff specification.
- Created isolated worktree branch `agent/phase4s-actor-knowledge-strategic-intent` from exact merged `main` base `24c4ca0801bfe56384345e41bdbc0d54fb9c2694`; pre-existing audit-report edits in the primary worktree remain untouched.
- Architecture choice: fixed-size per-actor typed memory with stable eviction; no event-history copy, arbitrary JSON, remote runtime snapshot, or parallel action planner. Strategic intent remains a structured context layer above the Phase 4R planner.
- GitHub publication is currently blocked by an invalid local `gh` token; local implementation and verification continue, but Draft PR/CI evidence require re-authentication before final handoff.
- Implemented typed observations, fixed-capacity actor memory, 12-turn stale source-failure avoidance, coarse threat sightings, public Apex memory, and strict no-snapshot save validation. Observation writes are terminal-gated and actor-owned.
- Implemented structured StrategicIntent with explicit COMMIT/PRESERVE/REEVALUATE/COMPLETE/INVALIDATE results. It supplies only source/zone preference context; all actions continue through `runNpcTurn` and existing actor-action services.
- Added S-1–S-15 coverage in `tests/phase4sKnowledgeIntent.test.ts`; representative source-failure and cautious-threat chains use `runNpcTurn`. Phase 4R/AF1, Phase 4O alternative victory, zero-stamina and debug privacy focused suites pass.
- Independent save audit now rejects 119/119 malformed cases with 0 construction failures. Dependency audit passes with R1–R4 all zero; NPC inventory maintenance was extracted so the largest core/data file is exactly 500 lines.
- Final local verification: typecheck/build PASS; full Vitest 118 files / 1,719 tests PASS; S-1–S-15 PASS; save audit 119/119 malformed rejected; dependency audit scans 124 files with R1–R4 zero; browser/repository secret scans PASS; production dependency audit reports 0 vulnerabilities.
- Exact `PHASE4S` regression generated `reports/phase4s-regression.json/.md`: requested=actual=500, trustworthy=500/500, regressionGate/engineHealthy true, and every hard engine-health counter is zero. Machine-readable cognition sanity records 495,849 observations, 77,088 deterministic evictions (15.5%), 18,763 commits vs 133,804 preserves, 15,845 reevaluations, 9,737 completions, 6,526 invalidations, 5,552 remembered source failures, and 27 Apex-contest intents. Threat-avoidance commits are 0 in this random matrix and are covered by deterministic S-12 instead of being inflated artificially.
- Browser smoke reached the live gameplay screen, `render_game_to_text` returned a valid playing state, normal UI exposed no private cognition, and the debug-only Actor Cognition summary was visually inspected with no console/page error artifact.
- Local candidate provenance is intentionally ignored and absent on this machine: exact local `art:validate` / `art:audit:phase4a` reported missing candidate metadata, while their built-in clean-checkout `CI=true` published-byte mode passed manifest/provenance/candidate-hygiene/runtime checks. No PNG, manifest, content, balance, dependency, or old-save migration change was made.
- Draft PR #26 is OPEN/DRAFT/UNMERGED. Implementation head `7977f05320b2516b15a59f39c2bcd7269be32cbb` passed CI run `31837716641` / job `94887631577`; final code+report evidence head `35e449b45d109244abdf37bd1ef06f91e7027167` passed CI run `31838053482` / job `94888682638`, both `completed/success`.

## Phase 4T — Localized Dynamic Incidents & Opportunity Windows (2026-08-18)

- Original Phase 4T prompt is the attached localized-incident, opportunity-window, information-boundary, T-1–T-16, full-gate and 500-game handoff specification.
- Created branch `agent/phase4t-localized-incidents-opportunities` from the exact Phase 4S merged `main` base `61d3ad6ce1434e46454b94b102b871ee4577b084`; baseline recorded at 118 files / 1,719 tests, 119-case save audit, dependency R1–R4 zero, and unchanged PNG/package state.
- Architecture: `IncidentDefinition` registry (4 archetypes: factory salvage reward pool, hospital emergency facility-overlay, underground maintenance access window, lab containment risk/reward) separated from a flat per-match `IncidentRuntime`. Scheduling resolves from a dedicated `phase4t:schedule:<seed>` RNG stream so seeds vary the local opportunities without perturbing established loot/spawn streams; the whole lifecycle is `state.time` driven inside `tickIncidents`.
- Visibility boundary: `PUBLIC_BROADCAST` incidents emit coarse `INCIDENT_ACTIVATED/RESOLVED/EXPIRED` events plus `incident_observed` memory to every alive actor; `LOCAL_DISCOVERY` incidents emit no global event and are learned only by physically entering the zone. `observePublicGameEvent` deliberately has no `INCIDENT_*` case so the global event array can never leak a LOCAL incident.
- Memory/intent integration: `incident_observed` joins the existing capacity-32 `ActorKnowledgeMemory` with stable keys; a single thin `respond_to_incident` intent sits below recovery/formal goals/Apex/threat and above the gear-up fallback. Ordinary turns PRESERVE the committed intent; formal takeover INVALIDATEs it and maintenance never restores it.
- Effects keep temporary state out of permanent runtime: `effectiveFacilityCharges`/`consumeFacilityCharge` spend overlay charges before base charges, and `effectiveLandmarkLocked` only overrides a lock while an ACTIVE access window covers the landmark. Expiry removes overlays without "repairing" legitimately drained/disabled state.
- Reward integrity: finite `ItemStack[]` pools created through `createStack`, claimed via the shared `addItem`/`canAccept` path with deterministic lowest-UID selection; contention on an exhausted pool is counted, never duplicated, and expiry discards unclaimed stacks. The formal `RESOLVE_INCIDENT` command is fully wired into commandTypes/commands/gameEngine/commandHandlers, legalActions (`incident` category), action costs (3 stamina), events, save validation, NPC (`npcAi` + `npcDecide`), and the auto player.
- Save semantics: `saveValidation/incidents.ts` validates exact runtime shapes, legal status transitions, timestamp ordering, per-effect reward compatibility, resolvers, and definition references; `knowledge.ts` gained the `incident_observed` branch with visibility-compatible provenance checks. `auditSaveValidation` now rejects 132/132 malformed cases with 0 construction failures.
- UI: `IncidentPanel` shows only legally known incidents (coarse state, no reward counts/charges), `EventLog` exposes public incident events plus the player's own claim, and the `?debug=1` DebugPanel gained Incident-runtime and actor-incident-memory inspectors; `tests/phase4tUi.test.tsx` proves the normal/debug isolation.
- Added `tests/phase4tIncidents.test.ts` covering T-1–T-16 (determinism, hidden-schedule and remote-runtime pair equivalence, coarse broadcast, local discovery, stale-memory correction, autonomous `runNpcTurn` response, finite contention, expiry, intent stability, formal Apex takeover, save roundtrip, malformed rejection, zero-stamina/terminal freeze, and the 4R/4S remote boundary). `npcDecide.ts` stance helpers were extracted to `npcStance.ts` and `commandHandlers.ts` trimmed so `audit:deps` passes with the largest core/data file exactly 500 lines.
