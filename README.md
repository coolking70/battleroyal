...
> 项目代号：`zone-battle-royale` · 版本 `0.3.2`（Phase 3A-2 Pre-Phase4 final closure）
...
# 自动平衡模拟
# 默认 = --games-per-cell 50（8 角色 × 5 策略 = 40 格，共 2000 局）
# 指定总局数：npm run simulate -- --games 3000 --seed-prefix MY-RUN
npm run simulate
```
...
- **体力战略循环**：移动 / 攻击 / 合成消耗体力，普通动作受体力闸门约束；**逃跑自 Phase 2A 起为免费行动（体力 0）**，无可退安全区时可原地脱离，保证遭遇战中永远存在一个可执行出口，但逃跑仍消耗 1 个时间单位、有可退区域时失败还可能被追击，因此免费 ≠ 无代价。恰好 0 体力时防御也提供一次应急免费选择，1 点体力仍不足以支付完整防御成本。
- **终局收束**：阶段（开局 / 中局 / 终局）+ 终局衰竭 + 180 硬上限，对局必然在有限时间结束。
- **信息不完全**：地图只显示噪音分档与最后已知位置情报；同区对手仅「遭遇中」暴露精确数值。
- **制作目标**：玩家可显式设定制作目标，NPC 按 TTL 自动规划并复规划。
- **存档安全**：当前版本存档深度校验 + 同版本正式迁移（缺失的可选装备槽 / 固定地图区域）；
  其他版本的旧存档一律拒绝加载，既不静默迁移也不删除，由玩家自行清理。
- **自动平衡**：`npm run simulate` 批量模拟并校验角色胜率比 < 2.5 且无 0 胜率角色（8 角色 × 5 策略）。
...
- **平衡收敛（Phase 3 当时记录，4 角色阵容）**：`npm run simulate -- --games 2000` 的最高/最低非零胜率比由基线 **5.88** 收敛至 **2.20**，4 个独立种子全部通过 `< 2.5` 红线。
  ⚠️ 该数字属于 Phase 3 的 4 角色阵容，**不代表当前 8 角色阵容**；当前阵容的最新平衡数据见 `BALANCE_CHANGELOG.md` 与 `reports/`。
...
- **12 个固定区域**，连接关系写死并在模块加载时做对称性与连通性自检；原 6 个区域继续保留为旧存档兼容基线：

  legacy 六区的连接关系与新增区域的完整拓扑详见 `src/data/zones.ts`。
...
- 当前 **1829 个测试，全部通过**，分布在 126 个文件（第一阶段 71 → 第二阶段 128 → 第三阶段 424 → Phase 3A 491 → Phase 3A-1 527 → Phase 3A-2 546 → … → Phase 4X 1829）。
