# character/fighter/injured

- Hash: `730c3b8ee1ab18420925d85ac75ce10d6721e86cee3779ccdbcd04692f99f8b3`
- Model: `agnes-image-2.1-flash`
- Requested size: 768x1024
- Requested ratio: 3:4
- Revision: 1
- Style profile version: `2c38f46d3530`

## Prompt

# Master style v1

Original urban survival game artwork, semi-realistic anime illustration, clean silhouettes, restrained desaturated palette, subtle painterly texture, high readability for game UI, dramatic but grounded lighting, consistent proportions, original costume design, cold strategic atmosphere, no recognizable commercial intellectual property.

The visual language is Japanese survival suspense blended with urban ruins and a restrained strategy interface. Prefer legible shapes, a clear focal subject, believable wear, and controlled contrast. Avoid photorealism, chibi proportions, and spectacle that would overpower the game UI.

# Character style

Game character portrait, waist-up or knee-up single subject, frontal or slight three-quarter view, complete head and clothing, clear face, readable silhouette, simple unobtrusive background. Anime semi-realism with light cel shading and soft painterly edges. Keep age, hairstyle, clothing construction, and signature prop consistent across variants.

Character design source of truth:
# Fighter design sheet

- Age range: late 20s to mid 30s.
- Face and hair: sturdy face, short practical hair, focused direct expression.
- Clothing: athletic layered streetwear with light protective pads and worn gloves; built for close combat, not a knight or armored soldier.
- Palette: charcoal, rust red, muted warm orange accent.
- Signature prop: wrapped forearm or compact improvised melee guard.
- Avoid: giant muscles, heavy armor, medieval weapons, military insignia, logos, text.

Asset brief:
The same fighter character design, clothing, hair, and wrapped forearm as the normal portrait; only lightly injured, dusty, fatigued, with minor clothing wear and a tense expression.

Variant: injured. Keep the entity identity stable across variants.

Technical composition: 768x1024, clear focal subject, no text, no logo, no watermark.

## Negative prompt

text, watermark, logo, signature, extra fingers, extra limbs, deformed hands, duplicate person, cropped head, extreme fisheye, low resolution, blur, UI frame, photo watermark, recognizable commercial IP, artist imitation
