...
export const LANDMARKS: readonly LandmarkDef[] = [
  ...
  { id: 'residential_apartment_block', zoneId: 'residential', name: '公寓楼', description: '几间被撬开的住宅仍藏着日用品。', icon: '🏢', kind: 'landmark', searchable: true, maxSearches: 3, initialLoot: [{ itemId: 'cloth', count: 1 }, { itemId: 'water', count: 1 }, { itemId: 'glass', count: 1 }], searchProfile: { preferredItemIds: ['cloth', 'water'], encounterChance: 0.12, riskDamage: 0 } },
  ...
  { id: 'factory_assembly_line', zoneId: 'factory', name: '装配线', description: '停摆的流水线上还能拆出少量完整零件。', icon: '🏭', kind: 'landmark', searchable: true, maxSearches: 3, initialLoot: [{ itemId: 'metal_parts', count: 1 }, { itemId: 'wire', count: 1 }, { itemId: 'iron', count: 1 }], searchProfile: { preferredItemIds: ['metal_parts', 'wire'], encounterChance: 0.22, riskDamage: 2, riskStatus: 'wild' } },
  ...
  { id: 'lab_analysis_terminal', zoneId: 'lab', name: '分析终端', description: '终端可以解析有限的研究线索，但每次启动都在消耗备用电源。', icon: '🖥️', kind: 'facility', searchable: true, maxSearches: 2, initialLoot: [{ itemId: 'battery', count: 1 }, { itemId: 'circuit', count: 1 }], searchProfile: { preferredItemIds: ['battery', 'circuit'], encounterChance: 0.3, riskDamage: 3, riskStatus: 'wild' }, interaction: facility('analyze', '分析样本', 'analyze', 2) },
  ...
  { id: 'underground_sealed_passage', zoneId: 'underground', name: '封闭通路', description: '通往深处的闸门需要服务机房先恢复。', icon: '🚪', kind: 'landmark', searchable: true, maxSearches: 2, initialLoot: [{ itemId: 'scrap', count: 1 }, { itemId: 'chemical_mix', count: 1 }], searchProfile: { preferredItemIds: ['scrap', 'chemical_mix'], encounterChance: 0.48, riskDamage: 6, riskStatus: 'wild' }, interaction: facility('sealed_passage', '开启封闭通路', 'start_generator', 1, { requiredLandmarkId: 'underground_service_room', requiresUnlock: true }) },
];
...
export function validateLandmarkRegistry(): string[] {
  ...
  for (const landmark of LANDMARKS) {
    ...
    if (landmark.interaction && landmark.interaction.maxCharges <= 0) errors.push(`设施 ${landmark.id} 的 maxCharges 非法`);
  }
  return errors;
}
