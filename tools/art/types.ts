...
export type PromptStrategy = 'standard' | 'character-positive-only' | 'character-combat-positive-only' | 'environment-positive-only' | 'item-positive-only-unmarked' | 'item-positive-only' | 'event-positive-only';
...
export interface ArtTask {
  ...
  postureOnly?: boolean;
  signaturePropMode?: 'static' | 'transition' | 'wearable';
  handsEmpty?: boolean;
  styleProfile: string;
  revision: number;
