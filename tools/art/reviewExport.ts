...
const REVIEW_REMINDERS: Record<string, string> = {
  ...
  'character/scout/combat': 'Compare beside the official Scout Portrait and Injured portrait. Check the single-prop state transition: the one raised binocular pair remains connected to one neck strap, with the chest beneath it clear. Record any duplicated signature prop for human review; never auto-reroll.',
};
const REVIEW_CHECKLISTS: Record<string, string[]> = {
  ...
  'character/scout/combat': ['clearly reads as the same Scout', 'same age range and hairstyle', 'same slate-blue jacket identity', 'same charcoal shirt / khaki outfit language', 'same side pouch identity', 'exactly one logical binocular prop is visible', 'raised binoculars are clearly the pair attached to the neck strap', 'no second pair remains hanging on the chest', 'active / tense state differs from Portrait', 'clearly differs from Injured state', 'no fixed weapon', 'no military/tactical redesign', 'no drastic redesign'],
};
