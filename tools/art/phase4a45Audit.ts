...
function buildRuntimeUsage(manifest: ArtManifest, tasks: readonly ArtTask[]): Record<string, unknown> {
  ...
  const surfaces = [
    ...
    { component: 'EncounterPanel', getter: 'getCharacterVisual + resolveCharacterVisualState', variants: ['portrait', 'injured', 'combat'], condition: 'visible opponent during encounter; no hidden remote NPC state', fallback: 'VisualImage formal → SVG → emoji' },
